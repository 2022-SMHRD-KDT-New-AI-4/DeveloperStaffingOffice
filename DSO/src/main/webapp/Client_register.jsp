...
<body>
  ...
  $('#joblist').change(function(){
	  ...
	  var list;
	  if(data=='UX기획'){
		  list = ['','웹 · 모바일 기획', '프로그램 · 기타 기획'];
	  }else if(data=='웹'){
		  list = ['','홈페이지','랜딩페이지','프론트엔드 · 퍼블리싱','검색 최적화 · SEO','애널리틱스','홈페이지 수정 · 유지보수'];
	  }else if(data=='커머스'){
		  list = ['','쇼핑몰', '쇼핑몰 수정 · 유지보수'];
	  }else if(data=='모바일'){
		  list = ['','앱','앱 수정 · 유지보수'];
	  }else if(data=='프로그램'){
		  list = ['','비지니스 애플리케이션','PC · 웹 프로그램','백엔드 · 서버','봇 · 챗봇'];
	  }else if(data=='트랜드'){
		  list = ['','노코드 · 로우코드','메타버스','블록체인 · NFT'];
	  }else if(data=='데이터'){
		  list = ['','데이터 구매 · 구축','데이터 마이닝 · 크롤링','데이터 전처리','데이터 라벨링','데이터 분석 · 시각화','인공지능 · 머신러닝','데이터베이스'];
	  }else if(data=='언리얼'){
		  list = ['','2D · 3D게임','AR · VR'];
	  }else if(data=='기타'){
		  list = ['','하드웨어 · 임베디드','보안','QA · 테스트','컴퓨터 기술지원','파일변환','기타'];
	  }
